...
def download_and_process_texas_tech():
    """Download Texas Tech logo and process it like get_images.py does"""
    
    logo_url = "https://a.espncdn.com/i/teamlogos/ncaa/500/2641.png"
    college_dir = "/app/get_images/sport_logos/college"
    
    print(f"Downloading Texas Tech logo from {logo_url}")
    
    try:
        # Download the logo
        img_path_png = os.path.join(college_dir, "TTU.png")
        ...
        print(f"✓ Successfully processed TTU.bmp with LED display optimization")
        return True
...
def main():
    print("Downloading and processing Texas Tech logo (team ID 2641) with LED display optimization...")
    success = download_and_process_texas_tech()
    
    if success:
        print("✓ Texas Tech logo processed successfully with same method as get_images.py!")
    else:
        print("❌ Failed to process Texas Tech logo")
